...
while true; do
  ...
  case "$choice" in
    ...
    6)
      bash Command/installSingbox.sh && \
      bash Command/confReality.sh && \
      bash Command/confSingbox.sh || echo "[ERROR] Reality deploy failed" ;;
    7)
      bash Command/uninstallSingbox.sh || echo "[ERROR] sing-box uninstall failed" ;;
    0)
      exit 0 ;;
    *)
      echo "Invalid choice." ;;
